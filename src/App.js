...
function App() {
  ...
  const [showSimpsonOnly, setButton]= React.useState(false)

  const buttonHandler = ()=> {
    setButton(!showSimpsonOnly)
  }


  return (
    <>
      <button onClick={buttonHandler}>Simpsons family Only: { showSimpsonOnly ? 'Off' : 'On'}</button>
      <div>
        {quoteList
        .filter(person => !showSimpsonOnly || person.character.includes('Simpson'))
        .map((quote, index) => (
